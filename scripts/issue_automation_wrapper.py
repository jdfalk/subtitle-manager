...
class TaskIntegration:
    ...
    def run_issue_analysis(
        self, repo_owner: str, repo_name: str, github_token: str, **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if specific_issue:
                ...
                results.update(self._analyze_single_issue(analyzer, specific_issue))
            else:
                # Analyze all high-priority issues
                results.update(
                    self._analyze_high_priority_issues(
    ...
    def _analyze_single_issue(
        self, analyzer: IssueAnalyzer, issue_number: int
    ) -> Dict[str, Any]:
    ...
    def _analyze_high_priority_issues(
        self, analyzer: IssueAnalyzer, automation: IssueAutomation, min_priority: int
    ) -> Dict[str, Any]:
        """Analyze all high-priority issues"""
        import requests
    ...
    def _generate_summary_stats(
        self, high_priority_issues: List[Dict]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "highest_priority": max(
                high_priority_issues, key=lambda x: x["priority_score"]
            ),
...
def main():
    ...
    parser.add_argument(
        "command", choices=["analyze", "status", "help"], help="Command to execute"
    )
    parser.add_argument(
        "--repo-owner", default="jdfalk", help="Repository owner (default: jdfalk)"
    )
    parser.add_argument(
        "--repo-name",
    ...
    parser.add_argument(
        "--issue-number", type=int, help="Analyze specific issue number"
    )
    parser.add_argument(
        "--min-priority",
        type=int,
    ...
    if args.command == "status":
        ...
        if updates_dir.exists():
            ...
            processed_updates = (
                list(processed_dir.glob("*.json")) if processed_dir.exists() else []
            )
                ...
                for update_file in pending_updates[:5]:  # Show first 5
                    try:
                        with open(update_file) as f:
                            data = json.load(f)
                        print(
                            f"  - {update_file.name}: Issue #{data.get('issue_number', 'unknown')}"
                        )
                    except Exception:
...
def _print_summary_results(results: Dict[str, Any]):
    ...
    if results.get("analysis_type") == "single_issue":
        ...
        if stats and stats.get("needs_immediate_attention", 0) > 0:
            print(
                f"⚠️  Critical Issues Requiring Immediate Attention: {stats['needs_immediate_attention']}"
            )

        if results["high_priority_issues"]:
            print("\nTop Priority Issue:")
            top_issue = max(
                results["high_priority_issues"], key=lambda x: x["priority_score"]
            )
            print(f"  #{top_issue['number']}: {top_issue['title']}")
            print(
                f"  Priority: {top_issue['priority_score']} ({top_issue['urgency_level']})"
            )
