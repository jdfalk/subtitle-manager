...
class IssueAnalyzer:
    ...
    def _analyze_content(self, title: str, body: str) -> Dict[str, Any]:
        ...
        if any(word in content for word in ['bug', 'error', 'fail', 'broken', 'crash']):
            issue_type = "bug"
        elif any(word in content for word in ['feature', 'enhancement', 'add', 'implement']):
        ...
        # Security detection
        if any(word in content for word in ['security', 'vulnerability', 'cve', 'exploit']):
    ...
    def _calculate_priority_score(self, issue_data: Dict, analysis: Dict, 
                                existing_labels: List[str]) -> int:
        ...
        if issue_type == 'bug':
            ...
            factors.append(f"Enhancement: +{self.priority_weights.get('enhancement', 40)}")
    ...
    def _generate_recommendations(self, issue_data: Dict, 
        ...
        if missing_labels:
            recommendations.append(
                f"Add labels: {', '.join(missing_labels)}"
            )
        ...
        # Module assignment
        if analysis['suggested_module'] and analysis['suggested_module'] not in current_labels:
            recommendations.append(
                f"Assign to module: {analysis['suggested_module']}"
            )
...
class IssueAutomation:
    ...
    def process_high_priority_issues(self, analyzer: IssueAnalyzer, 
                                   min_priority: int = 60) -> List[str]:
        """
...
def main():
    ...
    parser.add_argument("--analyze-issue", type=int, help="Analyze specific issue number")
    parser.add_argument("--process-high-priority", action="store_true", 
    ...
    if args.analyze_issue:
        ...
        update_ids = automation.process_high_priority_issues(analyzer, args.min_priority)
