...
func Handler(db *sql.DB) (http.Handler, error) {
	...
	mux.Handle("/api/extract", extractHandler())
	mux.Handle("/api/scan", scanHandler())
	mux.Handle("/api/scan/status", scanStatusHandler())
...
// extractHandler extracts the first subtitle stream from a media file.
//
// The handler expects a POST request with a JSON body containing a "media"
// field specifying the path to the media container. On success the
// extracted subtitles are returned in SRT format with Content-Type set to
...
func extractHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil || req.Media == "" {
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		items, err := subtitles.ExtractFromMedia(req.Media)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		...
		if err := sub.WriteToSRT(w); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
	})
...
// It expects a JSON body containing provider, directory, lang and optional upgrade.
// On success the scan is executed asynchronously and HTTP 202 is returned.
func scanHandler() http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			scanner.ScanDirectory(r.Context(), req.Directory, req.Lang, req.Provider, p, req.Upgrade, workers, store)
		}()
		w.WriteHeader(http.StatusAccepted)
	})
}

// scanStatusHandler returns the current scan progress.
func scanStatusHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		// Return a simple status for now - this would need to be connected to actual scan progress
