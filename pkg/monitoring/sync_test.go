...
func TestGetMonitoringStatsReturnsError(t *testing.T) {
	store := &MockSubtitleStore{}
	monitor := &EpisodeMonitor{store: store, logger: logrus.NewEntry(logrus.New())}

	store.On("ListMonitoredItems").Return(nil, assert.AnError)
...
func TestRemoveFromMonitoringReturnsListError(t *testing.T) {
	store := &MockSubtitleStore{}
	monitor := &EpisodeMonitor{store: store, logger: logrus.NewEntry(logrus.New())}

	store.On("ListMonitoredItems").Return(nil, assert.AnError)

	err := monitor.RemoveFromMonitoring("/path/missing.mkv")

	assert.Error(t, err)
	store.AssertExpectations(t)
}

func TestRemoveFromMonitoringDeleteError(t *testing.T) {
	store := &MockSubtitleStore{}
	monitor := &EpisodeMonitor{store: store, logger: logrus.NewEntry(logrus.New())}

	items := []database.MonitoredItem{{ID: "item-3", Path: "/path/file.mkv"}}

	store.On("ListMonitoredItems").Return(items, nil)
	store.On("DeleteMonitoredItem", "item-3").Return(assert.AnError)

	err := monitor.RemoveFromMonitoring("/path/file.mkv")

	assert.Error(t, err)
	store.AssertExpectations(t)
}

func TestAddToMonitoringReturnsUpdateError(t *testing.T) {
	mediaPath := createTempMediaFile(t)
	store := &MockSubtitleStore{}
	monitor := &EpisodeMonitor{store: store, logger: logrus.NewEntry(logrus.New())}
	opts := SyncOptions{Languages: []string{"en", "es"}, MaxRetries: 2}
	...
	existing := database.MonitoredItem{ID: "item-4", Path: mediaPath, Languages: string(existingLanguages), MaxRetries: 1}

	store.On("ListMonitoredItems").Return([]database.MonitoredItem{existing}, nil)
	store.On("UpdateMonitoredItem", mock.Anything).Return(assert.AnError)

	err = monitor.addToMonitoring(database.MediaItem{ID: "media-8", Path: mediaPath}, opts)
