...
func TestSingleFileJob_MetadataAndQueueMessage_PopulatesFields(t *testing.T) {
	t.Run("metadata", func(t *testing.T) {
		...
		if !strings.Contains(job.Description(), "/tmp/input.srt") {
			t.Fatalf("expected description to include input path, got %s", job.Description())
		}
...
func TestBatchFilesJob_MetadataAndQueueMessage_PopulatesFields(t *testing.T) {
	t.Run("metadata", func(t *testing.T) {
		...
		if !strings.Contains(job.Description(), "2") {
			t.Fatalf("expected description to include file count, got %s", job.Description())
		}
