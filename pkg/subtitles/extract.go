...
// ExtractFromMediaTrack extracts the specified subtitle stream from the given
// media container using the `ffmpeg` command line tool. The resulting subtitle
// items are returned. The `ffmpeg` binary must be available in $PATH.
func ExtractFromMediaTrack(mediaPath string, track int) ([]*astisub.Item, error) {
	tmp, err := os.CreateTemp("", "subextract-*.srt")
	if err != nil {
		return nil, err
	}
