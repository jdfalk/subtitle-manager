...
from subtitle_manager_sdk.exceptions import (
    SubtitleManagerError, AuthenticationError, RateLimitError
)
...
class SubtitleManagerIntegration:
    ...
    def __init__(self, base_url: str = "http://localhost:8080", api_key: Optional[str] = None):
        ...
        logger.info(f"Initialized Subtitle Manager integration with base URL: {base_url}")
    
    async def health_check(self) -> bool:
        """Check if the Subtitle Manager API is healthy."""
        try:
            system_info = self.client.get_system_info()
            logger.info(f"Health check passed - System: {system_info.os} {system_info.arch}")
            logger.info(f"Disk usage: {system_info.disk_free_formatted}/{system_info.disk_total_formatted}")
            return True
    ...
    def authenticate_session(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Authenticated as {login_response.username} with role: {login_response.role}")
            return True
        except AuthenticationError as e:
            logger.error(f"Authentication failed: {e}")
            return False
    
    def process_subtitle_file(self, file_path: Path, target_language: Optional[str] = None) -> Optional[bytes]:
        """
    ...
    def batch_download_subtitles(self, media_files: List[Dict[str, str]]) -> Dict[str, Dict]:
        """
        ...
        logger.info(f"Batch download completed: {success_count}/{total_files} successful")
    ...
    def analyze_download_history(self, days: int = 7) -> Dict:
        ...
        try:
            ...
            for item in all_items:
                # Count by type
                analysis['by_type'][item.type] = analysis['by_type'].get(item.type, 0) + 1
                
                # Count by status
                analysis['by_status'][item.status] = analysis['by_status'].get(item.status, 0) + 1
                
                # Count by provider (for successful downloads)
                if item.provider and item.status == OperationStatus.SUCCESS:
                    analysis['by_provider'][item.provider] = analysis['by_provider'].get(item.provider, 0) + 1
                ...
                analysis['most_active_days'][day_key] = analysis['most_active_days'].get(day_key, 0) + 1
            ...
            logger.info(f"Analyzed {analysis['total_operations']} operations from last {days} days")
    ...
    def extract_and_translate_pipeline(self, video_file: Path, target_languages: List[str]) -> Dict[str, Optional[bytes]]:
        """
        ...
        try:
            logger.info(f"Starting extraction and translation pipeline for: {video_file}")
...
"""
            ...
            if error_logs:
                ...
                for log in error_logs[:10]:  # Show last 10 errors
                    report += f"- {log.timestamp_date}: {log.component} - {log.message}\n"
