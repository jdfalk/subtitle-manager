...
class SubtitleManagerClient:
    ...
    def _handle_response(self, response: requests.Response) -> None:
        ...
        if response.status_code == 401:
            ...
            raise RateLimitError(
                message, retry_after_int, response.status_code, error_code
            )
        else:
    ...
    def login(self, username: str, password: str) -> LoginResponse:
        ...
        response = self._make_request(
            "POST", "/api/login", json_data={"username": username, "password": password}
        )
    ...
    def convert_subtitle(
        self, file: Union[str, BinaryIO], filename: Optional[str] = None
    ) -> bytes:
        """
    ...
    def translate_subtitle(
        ...
        if isinstance(file, str):
            with open(file, "rb") as f:
                ...
                response = self._make_request(
                    "POST", "/api/translate", files=files, data=data
                )
        else:
            if not filename:
                raise ValidationError("filename required for file-like objects")
    ...
    def extract_subtitles(
        ...
        if isinstance(file, str):
            with open(file, "rb") as f:
                ...
                response = self._make_request(
                    "POST", "/api/extract", files=files, data=data
                )
        else:
            if not filename:
                raise ValidationError("filename required for file-like objects")
    ...
    def download_subtitles(
        self, path: str, language: str, providers: Optional[List[str]] = None
    ) -> DownloadResult:
        """
    ...
    def wait_for_scan_completion(
        self, poll_interval: int = 5, timeout: int = 3600
    ) -> ScanStatus:
        """
